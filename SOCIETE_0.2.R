...
setwd("C:\\Users\\TEMP.Main.001\\Documents\\GitHub\\Hackerearth_projects\\SOCIETE")
...
# unique(dset$currency)
# 
# curs <- c('USD','GBP', 'CHF', 'EUR', 'JPY')
# factrs <- factor(curs)
# dset$currency <-as.factor(factrs)
...
bstSparse <- xgboost(data = data, label = dlabel, max.depth = 6, gamma = 0.001, eta = 0.01, nround = 1000, eval.metric = "rmse", print_every_n = 50, objective = "reg:linear")
...
# renc1 <- LabelEncoder.fit(dset$pf_category)
# dset$pf_category <- transform(renc, dset$pf_category)
# dset$pf_category <- as.data.frame(inverse.transform(renc1,dset$pf_category))

# cur <- NULL
# transform(renc,dset[45, "sell_d"])
reverse <- as.data.frame(inverse.transform(renc, rates_merged$date))
# cur$id <- as.data.frame(dset$portfolio_id)
# cur <- data.frame(cur)
#make matrix for loop
rates_merged_m <- data.matrix(rates_merged, rownames.force = NA)


#check return and profit
for (i in 1:nrow(dset)) {
 if (sign(dset[i, 'return']) != sign(dset[i,'profit'])){
   print(i)
  } else {
   o <- rbind(i)
  }
...
#check data is numeric for xgboost
for (i in 1:ncol(dset)) {
  
  if (is.numeric(dset[,i]) == FALSE) {
  print (i)
  }else {
      NULL
  }
...
# check if sell date < creation day

for (i in 1:nrow(dset)) {
  if (dset[i, 'sell_date'] < dset[i, 'creation_date'])  {
    print(i)
    o <- rbind(i)
  } else {
    NULL
...
for (i in 1:nrow(dset)) {
  ...
  if (dset[i,"currency"] == 'CAD') {
    dset[i,"sold"] <- dset[i, "sold"] * rates_merged[rates_merged[,'date'] == dset[i,'sell_d'], 'Value_cad']
    dset[i,"bought"] <- dset[i, "bought"] * rates_merged[rates_merged[, 'date'] == dset[i, 'buy_d'], 'Value_cad']
  } else if (dset[i,"currency"] == 'GBP') {
    dset[i,"sold"] <- dset[i, "sold"] * rates_merged[rates_merged[,'date'] == dset[i,'sell_d'], 'Value_gbp']
    dset[i,"bought"] <- dset[i, "bought"] * rates_merged[rates_merged[, 'date'] == dset[i, 'buy_d'], 'Value_gbp']
    } else if (dset[i,"currency"] == 'EUR') {
      dset[i,"sold"] <- dset[i, "sold"] * rates_merged[rates_merged[,'date'] == dset[i,'sell_d'], 'Value_eur']
      dset[i,"bought"] <- dset[i, "bought"] * rates_merged[rates_merged[, 'date'] == dset[i, 'buy_d'], 'Value_eur']
    } else if (dset[i,"currency"] == 'JPY') {
      dset[i,"sold"] <- dset[i, "sold"] * rates_merged[rates_merged[,'date'] == dset[i,'sell_d'], 'Value_jpy']
      dset[i,"bought"] <- dset[i, "bought"] * rates_merged[rates_merged[, 'date'] == dset[i, 'buy_d'], 'Value_jpy']
    } else if (dset[i,"currency"] == 'CHF') {
      dset[i,"sold"] <- dset[i, "sold"] * rates_merged[rates_merged[,'date'] == dset[i,'sell_d'], 'Value_chf']
      dset[i,"bought"] <- dset[i, "bought"] * rates_merged[rates_merged[, 'date'] == dset[i, 'buy_d'], 'Value_chf']
...
library(CatEncoders)
# dset <- data.frame(dset)
renc <- LabelEncoder.fit(rates_merged$date)
rates_merged$date <- transform(renc, rates_merged$date)
# cur <- NULL
# transform(renc,dset[45, "sell_d"])
reverse <- as.data.frame(inverse.transform(renc, rates_merged$date))
# cur$id <- as.data.frame(dset$portfolio_id)
...
#encode dates
dset_m <- data.frame(dset)
dset_m$sell_d <- transform(renc, dset_m$sell_d)
dset_m$buy_d <- transform(renc, dset_m$buy_d)

# make matrix for loop
dset_m <- data.matrix(dset_m, rownames.force = NA)
...
for (i in 1:nrow(dset)) {
  ...
  if (dset[i,"currency"] == 'CHF') {
    dset1[i,"sold"] <- dset[i, 'Value'] 
  } else {
    NULL
  }
...
for (i in 1:nrow(dset)) {
    ...
    if (dset[i,"currency"] == 'CHF') {
      dset[i,"sold"] <- dset[i, "sold"] * chf$Value[which(chf$Date==dset[i, 'sell_date'])]
      dset[i,"bought"] <- dset[i, "bought"] * chf$Value[which(chf$Date==dset[i, 'creation_date'])]
    } else if (dset[i,"currency"] == 'GBP') {
      dset[i,"sold"] <- dset[i, "sold"] * gbp$Value[which(gbp$Date==dset[i, 'sell_date'])]
      dset[i,"bought"] <- dset[i, "bought"] * gbp$Value[which(gbp$Date==dset[i, 'creation_date'])] 
    } else if (dset[i,"currency"] == 'EUR') {
      dset[i,"sold"] <- dset[i, "sold"] * eur$Value[which(eur$Date==dset[i, 'sell_date'])]
      dset[i,"bought"] <- dset[i, "bought"] * eur$Value[which(eur$Date==dset[i, 'creation_date'])]
    } else if (dset[i,"currency"] == 'JPY') {
      dset[i,"sold"] <- dset[i, "sold"] * jpy$Value[which(jpy$Date==dset[i, 'sell_date'])]
      dset[i,"bought"] <- dset[i, "bought"] * jpy$Value[which(jpy$Date==dset[i, 'creation_date'])]
    } else if (dset[i,"currency"] == 'CAD') {
      dset[i,"sold"] <- dset[i, "sold"] * cad$Value[which(cad$Date==dset[i, 'sell_date'])]
      dset[i,"bought"] <- dset[i, "bought"] * cad$Value[which(cad$Date==dset[i, 'creation_date'])]
    } else {
    NULL
    }
}
